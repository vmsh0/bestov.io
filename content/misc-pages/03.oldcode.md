---
title: 'bonding.c from Linux 2.2.15'
---
...
Paraphrasing, when a device driver enqueues a received packet into the network stack by calling `netif_rx()`, the packet will be "rebranded" as coming from the device saved into `skb->dev->slave` if the device has the `IFF_SLAVE` set. 
